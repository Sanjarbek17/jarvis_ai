...
### 2. Pull a Model
Ollama starts empty. You must manually pull the model you want to use. We recommend `qwen3:0.6b` for the Jarvis app:
...
> 3. Running `ollama run qwen3:0.6b` directly from your terminal.
...
| **Model not found** | Run `docker compose exec ollama ollama pull qwen3:0.6b` manually. |
| **High Resource Usage** | The `qwen3:0.6b` model is very lightweight, but ensure no other heavy LLMs are running. |
